...
public class TestStubReader {
	...
	public String setSheetAndGetData(String sheetName, String Rowheader, String colheader){
        
        setReadingSheet(sheetName);
        return getData(Rowheader,colheader);
 }
	...
	public void setSheetAndAddData(String sheetName, String header, String colheader,String Value){
        
        setReadingSheet(sheetName);
         setData(header,colheader,Value);
 }
	
	
	public void setData(String rowHeader,String colHeader,String Value){
		intiateWorkbook();
		String data=null;
		sheet=getSheet();
		Row currRow=null;
		boolean isHeadExisting=false;
		...
		for(int i=0;i<sheet.getPhysicalNumberOfRows();i++){
			...
			if(RowHeader.equals(rowHeader)){
				isHeadExisting=true;
				currRow.getCell(colDataNum).setCellType(CellType.BLANK);
				currRow.getCell(colDataNum).setCellValue(Value);
			}
		}
		if(!isHeadExisting){
		currRow.createCell(currRow.getPhysicalNumberOfCells()+1).setCellValue(colHeader);	
		currRow.createCell(currRow.getPhysicalNumberOfCells()+1).setCellValue(Value);	
		...
		//Update pdfdate in excel
		
		public static int getValueIndexCellFrmSheet(Sheet sheetname,String HeaderCellValue){
			int index=-1;
			Row currRow=sheetname.getRow(0);
			for(int j=1;j<currRow.getLastCellNum();j++){
				String valueForHeader=currRow.getCell(j).getStringCellValue();
				if(valueForHeader.equals(HeaderCellValue)){
					index=j;
		...
		public void updateExcelForPDF(String sheetName,String RowHeader,Map<String ,String> valStoreMap){
		try{
/*			FileInputStream file=new FileInputStream(new File(stubFileName));
			...
			Cell cell=null;
		String crrDate=null;
List<String> allColHeaders=new ArrayList<String>();
int colCount=sheet.getRow(0).getLastCellNum();
columnCount=colCount;
for(int i=0;i<colCount;i++){
	String colHea=setSheetAndGetData("PDFTestData","Scenerio",i);
	allColHeaders.add(colHea);
}
System.out.println("All colheader is : "+allColHeaders);
	
	for(String colHeader:allColHeaders){
	int colDataNum=getValueIndexCellFrmSheet(sheet,colHeader);
	for(int i=0;i<sheet.getPhysicalNumberOfRows();i++){
		...
		if(rowHeader.equals(RowHeader)){
			for(int j=1;j<currRow.getLastCellNum();j++){
				cell=currRow.getCell(colDataNum);
				if(valStoreMap.get(colHeader)!=null && cell.equals(valStoreMap.get(colHeader))){
					cell.setCellType(CellType.BLANK);
					cell.setCellValue(valStoreMap.get(colHeader).trim());
...
/*	file.close();
	FileOutputStream outFile=new FileOutPutStream(stubFileName);
