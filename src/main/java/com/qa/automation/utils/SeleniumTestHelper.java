...
private WebDriverWait driverWait;
	...
	public void waitForPageLoad(){
		...
		wait.until(new ExpectedCondition<Boolean>(){
			public Boolean apply(WebDriver driver){
				return ((JavascriptExecutor)driver).executeScript("retun document.readyState").equals("complete");
			}
		});
		}catch(Exception e){
					}}
	
	public boolean waitForElementPresent(WebElement ele,int TimeoutInSecond){
		boolean isElementPresent=false;
		for(int i=1;i<TimeoutInSecond*2;i++){
			...
			try{
				...
				log.info("element is not yet displayed "+ele+" still waiting..");
			}}
		return isElementPresent;
		}
	
	public boolean isElementPresent(WebElement element){
		boolean isElementPrst=false;
		try{
			waitForElementVisisble(element,20);
		...
		return isElementPrst;
	}
	
	public void waitForElementVisisble(WebElement element,int time){
		log.info("Checking the prsence of element "+element);
		WebDriverWait wait=new WebDriverWait(driver,10);
		wait.until(ExpectedConditions.visibilityOf(element));
	}
	
	public void customWait(WebElement element,int time) throws Exception{
		try{
			log.info("Checking the prsence of element "+element);
	...
	public boolean scrollIntoElementView(WebElement element){
		try{
	...
	public void viewElementByJavaScriptExecutor(WebElement element){
		try{
			JavascriptExecutor executor=((JavascriptExecutor)driver);
	...
	public void clickElementByJavaScriptExecutor(WebElement element){
		...
		log.info("Exception occured in JavaScriptFunction "+element);	
...
/*	public static boolean verifyElement(By locator) {
		...
		WebDriverDispatcher wpatch = (WebDriverDispatcher)driver;
		try {
...
/*	public static boolean verifyElement(By locator, int maxWaitTime) {
	...
	public static WebElement locateRow(String description) {
		...
		SeleniumTestHelper.waitForElementToBeDisplayed(driver, driver.findElement(By.partialLinkText(description)), 60);
		return driver.findElement(By.partialLinkText(description));
	}
	
	public static WebElement dropDownPartialItem(String partialItemName){
		return driver.findElement(By.xpath("//li[contains(@class,'select2-results__option') and contains(text(),'"+partialItemName+"')]"));
	}
	
	public static WebElement dropDownItem(String itemName){
		return driver.findElement(By.xpath("//li[contains(@class,'select2-results__option') and text()='"+itemName+"']"));
	}

	public static void uploadFile(String filePath) {
		try {
			String script = ConfigFileReader.getProperty("applicationFolder")
...
//			}
	...
	public static String getAbsolutePath(String fileName){
	   	...
	   	String relativePath = ConfigFileReader.getProperty("attachmentsPath")+ConfigFileReader.getProperty(fileName);
	...
	public static File getLastDownloadedFile(){
		...
		long lastModified = 0;
		String downloadDir = ConfigFileReader.getDownloadLocation();
		do{
	...
	/**
	 * @author Dhaval
	 * @param Method add desired days in current date
	 */
...
/**
 * @author Dhaval
 * @param Method generate random numerical string
 */
